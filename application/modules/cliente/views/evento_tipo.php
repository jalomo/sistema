...
<div class="container-fluid show-top-margin separate-rows tall-rows tamano1500 fondo_pantalla">
  ...
  <div class="row show-grid">
   ...
   .eventos_con{ height:765px;
    ...
    <div class="eventos_con" align="center">
            	<br/>
                <br/>
            <div align="center" class="textos_al" >Selecciona el tipo de boleto.  </div> 
            <br/>
            
            
             <?php echo form_open('cliente/save_tipo/'.$id_evento, array('id'=>'')); ?>
           <div>
           <?php //print_r($usuario);?>
           
           <?php if($usuario->usuarioSexo==''):?>
           
           <input type="radio" name="tipo" value="Hombre" checked/><label>HOMBRE</label>
           &nbsp;
           &nbsp;
           &nbsp;
           &nbsp;
           &nbsp;
           <input type="radio" name="tipo" value="Mujer"/><label>MUJER</label>
          <?php else:?>
          
          <?php if($usuario->usuarioSexo=='Hombre'){?>
          	<input type="radio" name="tipo" value="Hombre" checked/><label>HOMBRE</label>
           <?php }else{?>
           	<input type="radio" name="tipo" value="Hombre" /><label>HOMBRE</label>
           <?php }?> 
           &nbsp;
           &nbsp;
           &nbsp;
           &nbsp;
           &nbsp;
           <?php if($usuario->usuarioSexo=='Mujer'){?>
           <input type="radio" name="tipo" value="Mujer" checked/><label>MUJER</label>
           <?php }else{?>
           <input type="radio" name="tipo" value="Mujer" /><label>MUJER</label>
           <?php }?>
