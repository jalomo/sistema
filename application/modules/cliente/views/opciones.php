...
/*  $("#save_user").submit(function(e){
  ...
  $("#save_oxxo").on('submit', function(e){
        myform = this;
        e.preventDefault();
       sendFormPagoOxxo();
  ...
  $("#save_paypal").on('submit', function(e){
        myform = this;
        e.preventDefault();
  ...
  $("#save_banco").on('submit', function(e){
        myform = this;
        e.preventDefault();
  ...
  $("#save_reservar").on('submit', function(e){
        myform = this;
        e.preventDefault();
  ...
  $("#save_contado").on('submit', function(e){
        myform = this;
        e.preventDefault();
  ...
  $("#change_option").change(function (event){
    ...
    au=$("#test_total").val();
    au=au*id;
    $("#total_precio_u").val(number_format(au, 2, '.', ''));
    $("#p_total").text(number_format(au, 2, '.', ''));
    $("#oxxo_htp_monto").val(number_format(au, 2, '.', ''));
    //alert($("#p_total").text());
...
<div id="content" base ="<?php echo(base_url())?>" ></div>
...
<div class="container-fluid show-top-margin separate-rows tall-rows tamano1500 fondo_pantalla">
  ...
  <div class="row show-grid">
    ...
    <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_pagos')); ?> 
    
    <input  type="hidden" name="tipo[pago]" value="1" id="tip_pag"/>
    <br/>
     ...
     <hr/>
        ...
        <div align="center">
          ...
          <br/>
          <br/>
         <!--input type="radio" name="opciones" value="1" checked class="indicador"/--><label class="textos_al">PAGAR</label>
         <br/>
         ...
         <tr>
           ...
           <td>
              <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_banco')); ?> 
              <!--input  type="radio" name="tipo[pago]" value="1" title="DEPOSITO BANCARIO"/-->
			   <button type="submit" style="width:100px; height:60px; margin:5px;" class="btn btn-default">
			   <?php echo img(array('src'=>'statics/img/banco.png','width'=>'20px')); ?>
               </button> 
            ...
            <td>
               <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_paypal')); ?> 
               <!--input  type="radio" name="tipo[pago]" value="2" title="PAYPAL"/-->
			   <button type="submit" style="width:100px; height:60px; margin:5px;" class="btn btn-default"> 
			   <?php echo img(array('src'=>'statics/img/paypal.png','width'=>'50px')); ?>
               </button>
            ...
            <td>
              <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_oxxo')); ?> 
              <!--input  type="radio" name="tipo[pago]" value="3" title="OXXO"/-->
			  <button type="submit" style="width:100px; height:60px; margin:5px;" class="btn btn-default">
			  <?php echo img(array('src'=>'statics/img/oxxo.png','width'=>'50px')); ?>
              </button>
             ...
             <!--td>
             	<?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_contado')); ?> 
               
			   <button type="submit" style="width:100px; height:60px;margin:5px;" class="btn btn-default">
			   <?php echo img(array('src'=>'statics/img/llamar.png','width'=>'20px')); ?>
               </button>
         ...
         <br/>
         <hr/>
         <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_reservar')); ?>
         <!--input type="radio" name="opciones" value="2" class="indicador"/-->
         <button type="submit" style="height:60px;margin:5px;" class="btn btn-default">
         <label class="textos_al">RESERVAR</label>
         </button>
          <?php echo form_close(); ?>
         <br/>
         <br/>
         <hr/>
         <?php echo form_open('cliente/save_tipo_pago/', array('id'=>'save_rp')); ?>
         <!--input type="radio" name="opciones" value="2" class="indicador"/-->
         <button type="submit" style="height:60px;margin:5px;" class="btn btn-default">
         <label class="textos_al">RPs</label>
         </button>
          <?php echo form_close(); ?>
       </div>
       <br/>
       ...
       <div align="center"><!--button type="submit" id="guarda_boton" class="esconder_  btn btn-primary"  style="font-size:18px; width:200px; height:30px;">ACEPTAR</button--></div>
       <br/>
        

      <input type="hidden" value="<?php echo $pecio_total;?>" name="pecio_total">
      <input type="hidden" value="<?php echo $nombre_ciudad;?>" name="nombre_ciudad">  
      <input type="hidden" value="<?php echo $precio_ciudad;?>" name="precio_ciudad">
      <input type="hidden" value="<?php echo $evento_id;?>" name="evento_id">
      <input type="hidden" value="<?php echo $nombre_evento;?>" name="nombre_evento">
      <input type="hidden" value="<?php print_r( $modificadores);?>" name="modificadores">
      <input type="hidden" value="<?php echo $userId;?>" name="userId">
      <input type="hidden" value="<?php print_r ($dato_evento);?>" name="dato_evento">
      ...
      <!-- INICIO FORMULARIO OXXO -->
       <form  method="POST" action="https://www2.oxxo.com:8443/HTP/oxxo/request" id="frmOxxoHtp">
      <input type="hidden" id="oxxo_htp_referencia" name="oxxo_htp_referencia" value="8861811407"/>
       <input type="hidden" id="oxxo_htp_monto"          name="oxxo_htp_monto" value="<?php echo number_format($pecio_total, 2, '.', '');?>"/>
